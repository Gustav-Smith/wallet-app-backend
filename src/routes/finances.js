  ...
  try {
    ...
    if (!date || date.length != 10) {
      return res.status(400).json({
        error: "Date is mandatory and should be in the format yyy-mm-dd.",
      });
    }
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    if (email.length < 5 || !email.includes("@")) {
      return res.status(400).json({ error: "E-mail is valid." });
    }
    ...
    if (financeItemQuery.rows[0].user_id !== userQuery.rows[0].id) {
      return res
        .status(401)
        .json({ error: "Finance row doest not belong to user" });
    }
...
router.get("/", async (req, res) => {
  try {
    ...
    const findDate = new Date(year, month + 1, 0).toISOString();
    ...
    const values = [userQuery.rows[0].id, initDate, findDate];
